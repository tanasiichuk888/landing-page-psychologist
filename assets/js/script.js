...
function Modality(beforeOpen) {
    ...
    if (typeof (this.beforeOpen) !== 'function') this.beforeOpen = function () {
        return true
    };


    document.addEventListener('click', function (e) {
        if (e.target.hasAttribute('data-mdlt-open')) root.open(e.target.dataset.mdltOpen);
        if (e.target.hasAttribute('data-mdlt-close'))(root.close.bind(root, e.target.dataset.mdltClose))();
    })

    this.open = function (name, time) {
        var element = document.querySelector('[data-mdlt=' + name + ']');

        if (element) {
            root.activeList[name] = document.body.appendChild(createModal(element, name));

            root.beforeOpen(root, name);
            ...
            if (time) setTimeout(root.close.bind(root, name), time);
    ...
    function createModal(el, name) {
        ...
        modal.className = 'modality';
        modal.onclick = function (e) {
            if (e.target == modal)(root.close.bind(root, name))();
        }
            ...
            dialog: {
                ...
                content: {
                    close: {
                        ...
                        click: root.close.bind(root, name)
                    },
        ...
        for (var prop in config) modal.appendChild(createElem(config[prop]));

        return modal;
    }

    function createElem(config) {
        if (config instanceof HTMLElement) return config;

        var elem = document.createElement(config.type);
        elem.className = config.class ? 'modality-' + config.class : null;
        ...
        if (config.append) {
            if (config.content instanceof HTMLElement) {
                ...
                for (var prop in config.content) {
                    elem.appendChild(createElem(config.content[prop]));
                }
            }
        } else {
            elem.innerHTML = config.content ? config.content : null;
        }

        if (config.click) elem.onclick = config.click;
...
var mdlt = new Modality(function (root, name) {
    var $modal = $(root.activeList[name]);
    $modal.find('[phone-mask]').inputmask('+7 (999) 999-99-99', {
        "clearIncomplete": true
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    $('[slick-slider]').slick({
        ...
        prevArrow: '<button type="button" class="slider-arrow prev"><i class="fas fa-chevron-left"></i></button>',
        nextArrow: '<button type="button" class="slider-arrow next"><i class="fas fa-chevron-right"></i></button>'
    });

    $('[collapse]').each(function () {
        $(this).on('click', '[collapse-head]', function () {
    ...
    $('[data-scroll]').on('click', function (e) {
        ...
        if ($(to).length > 0) $('body, html').animate({
            scrollTop: $(to).offset().top - $header.outerHeight()
        }, 1500);
        else console.log('scroll: [' + to + '] not found!');
    });
    ...
    $(window).on('scroll', function () {
        var $scrollTop = $('[scrolltop]'),
            toShow = $(window).scrollTop() > document.documentElement.clientHeight;

        toShow ? $scrollTop.addClass('active') : $scrollTop.removeClass('active');
    });
